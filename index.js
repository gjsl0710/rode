...
client.on('ready', () => {
  console.log('켰다.');
  client.user.setPresence({ game: { name: '아 모든일이 귀찮다' }, status: 'online' })
});
...
client.on('message', (message) => {
  ...
  if(message.content == '엠베드') {
    ...
    let helpImg = 'https://media.discordapp.net/attachments/717205181502324868/717207192695996557/cbjr21565491229_1.jpg';
    let commandList = [
      {name: '엠베드', desc: 'embed 예제1'},
      {name: '!t공지', desc: '텍스트형식 디엠 공지'},
      {name: '!공지', desc: '전체공지'},
      {name: '!청소', desc: '텍스트 지움'},
      {name: '!코드', desc: '서버 초대권'},
